...
import useRecommendationMovieList from "@/hooks/useRecommendationMovieList";
...
const Home = () => {
  ...
  const { data: trendingShows = [], isLoading: isLoadingShows } =
    useTrendingShowList();
  const {data: recommendationMovies =[], isLoading: isLoadingRecommendationMovies} = useRecommendationMovieList()
    ...
    if (
      ...
      return (
        <>
          ...
          <Billboard />
          <div className="pt-5">
            <MovieList title={`Recommendations based on your list`} data={recommendationMovies} />
