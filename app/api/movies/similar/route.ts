...
export async function GET(
  ...
  if (session) {
    ...
    const genre_ids = list?.movies[random]?.genres.map((genre:any) => genre.id)
    const options = {
      method: "GET",
      url: `https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=${genre_ids.join("%2C%20")}`,
      params: { language: "en-US" },
      headers: {
        accept: "application/json",
        Authorization: `Bearer ${process.env.TMDB_BEARER_TOKEN}`,
    ...
    let movies = await axios.request(options);
    return NextResponse.json({choice:  list?.movies[random], results: movies.data.results});
  }
}
